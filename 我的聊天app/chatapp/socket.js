...
function getsocket(server){
    ...
    await  db.usermodle.updateOne({socketid:socket.id},{isonline:"false",socketid:"null",enable_flag:"Y"},(err,docs)=>{
      ...
      socket.on('sendmsg',async (data)=>{  
       ...
       await db.userchatmodle.create({from:JSON.stringify(data.from),to:JSON.stringify(data.to),content:data.content,fromusername:data.from.username,tousername:data.to.username,time:data.time,enable_flag:"Y"})
      })


      socket.on('senddemsg',async (data)=>{
       try {
        await db.userchatmodle.updateOne({fromusername:data.fromusername,tousername:data.tousername,time:data.time},{enable_flag:'N'})

        let d=await db.usermodle.findOne({username:data.to.username,enable_flag:'Y'},(error)=>{
          if(error)
          {
            console.log(error);
          }
        ...
        socket.to(d.socketid).emit('demsg',data)
       } catch (error) {
        console.log(error);
       }
